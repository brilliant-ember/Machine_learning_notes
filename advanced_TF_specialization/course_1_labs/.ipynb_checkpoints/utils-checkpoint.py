## this is the test suite copied from github
# https://github.com/sasidhar-programmer/Tensorflow_Advance_Techniques/blob/main/1-custom_models_layers_losses/week-1/utf-8''utils.py
import numpy as np
...
def test_final_model(final_model):
    ...
    test_cases = [
        ...
        },
        {
            ...
            "error_message": 'quality layer has an incorrect activation. Please check implementation.'
        },
    ]
    ...
    test_cases = [
        {
            ...
            "error_message": 'quality metrics is incorrect. Please check implementation.'
        },
        {
            "name": "metrics_1_check",
            "result": (model.metrics[1].name == 'type_accuracy') or 
                      (model.metrics[1].name == 'type_binary_accuracy'),
            "expected": True,
            "error_message": f'classifcation type metrics: {model.metrics[1].name} is incorrect. Please check implementation.'
        },
        {
            "name": "type_loss_check",
            "result": (model.loss['type'] == 'binary_crossentropy') or 
                      (model.loss['type'].name == 'binary_crossentropy') or 
                      (str(model.loss['type']).split()[1] == 'binary_crossentropy'),
            "expected": True,
            "error_message": f'classification type loss: {model.loss["type"]} is incorrect. Please check implementation.'
        },
        {
            "name": "quality_loss_check",
            "result": (model.loss['quality'] in ['mse', 'mean_squared_error']) or 
                      (str(model.loss['quality']).split()[1] == 'mean_squared_error') or 
                      (model.loss['quality'].name == 'mean_squared_error'),
            "expected": True,
            "error_message": f'quality loss: {model.loss["type"]} is incorrect. Please check implementation.'
        },
    ]
